...
export default function Swipe() {
    ...
    return (
      <Layout showNav={false}>
        <div className="flex flex-col items-center justify-center flex-1 py-20 text-center h-screen px-4">
          ...
          <h1 className="text-4xl font-black mb-4">¡Ya los viste todos!</h1>
          <p className="text-muted-foreground mb-10 text-lg">
            Deslizaste todos los destinos disponibles.
          </p>
          <Button
            size="lg"
            className="h-16 text-lg rounded-2xl px-8 w-full max-w-xs shadow-xl shadow-primary/20 font-black"
            onClick={() => setLocation(`/groups/${groupId}/results`)}
          >
            Ver coincidencias del grupo
          </Button>
          <Button
            variant="ghost"
            className="mt-4 h-14 w-full max-w-xs text-muted-foreground font-bold"
  ...
  const bgImgUrl = activeDest.imageUrl || "/destinations/bali.png";

  return (
    <Layout
      showNav={false}
      className="py-2 overflow-hidden px-2 relative flex flex-col h-[100dvh] bg-background"
    >
      <div className="flex justify-between items-center px-4 py-4 z-10">
        <button
          onClick={() => setLocation(`/groups/${groupId}`)}
          className="p-3 bg-card/80 backdrop-blur border border-border/50 rounded-full text-foreground shadow-sm"
        >
        ...
        <div className="text-xs font-black uppercase tracking-wider bg-card/80 backdrop-blur border border-border/50 px-4 py-2 rounded-full shadow-sm text-primary">
          {unswipedDests.length - currentIndex} restantes
        </div>
      </div>
      ...
      <div className="flex-1 relative flex items-center justify-center mb-28 w-full max-w-sm mx-auto z-10">
        ...
        >
          ...
          <motion.div
            ...
            className="absolute top-10 right-10 z-20 transform rotate-12"
          >
          ...
          <motion.div
            ...
            className="absolute top-10 left-10 z-20 transform -rotate-12"
          >
          ...
          <div className="absolute bottom-0 left-0 right-0 p-8 text-white">
            <h2 className="text-4xl font-black mb-2 drop-shadow-lg leading-none">
              {activeDest.name}
            </h2>
            ...
            <p className="text-sm text-white/90 line-clamp-3 mb-5 leading-relaxed font-medium drop-shadow">
            ...
            <div className="flex flex-wrap gap-2">
              {activeDest.tags.map((tag) => (
                <span
                  key={tag}
                  className="px-3 py-1.5 bg-white/20 backdrop-blur-md rounded-lg text-xs font-black uppercase tracking-wide border border-white/30"
                >
                  {tag}
                </span>
              ))}
