...
export default function Onboarding() {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    createUser.mutate(
      { data: { displayName: name.trim(), username } },
      {
        onSuccess: (user) => {
    ...
    <Layout>
      <div className="flex flex-col flex-1 justify-center max-w-sm w-full mx-auto pb-32">
        <div className="text-left">
          <h1 className="text-4xl font-black mb-3">¿Cómo te llamás?</h1>
          <p className="text-muted-foreground mb-10 text-lg">
            Así te van a ver tus amigos en la app.
          </p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          >
            {createUser.isPending ? "Creando..." : "Continuar"}
          </Button>
        </form>
      </div>
    </Layout>
